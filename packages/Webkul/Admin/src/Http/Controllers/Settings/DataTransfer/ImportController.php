...
class ImportController extends Controller
{
    ...
    public function uploadProductImages(Request $request)
    {
        ...
        foreach ($request->file('images') as $image) {
            // Store each image in the storage/app/import/product-images directory
            $filename = $image->getClientOriginalName();
            $storedPath = $image->storeAs('import/product-images', $filename);
        }
        return redirect()->back()->with('success', 'Images uploaded successfully.');
    }

    public function uploadProductVideos(Request $request)
    {
        $request->validate([
            'videos' => 'required|array|max:20',
            'videos.*' => 'file|mimes:mp4,webm,mkv|max:5120', // Max file size: 5mb
        ]);
        foreach ($request->file('videos') as $video) {
            // Store each video in the storage/app/import/product-videos directory
            $filename = $video->getClientOriginalName();
            $storedPath = $video->storeAs('import/product-videos', $filename);
        }
