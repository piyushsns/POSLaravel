...
class Importer extends AbstractImporter
{
    ...
    public function validateRow(array $rowData, int $rowNumber): bool
    {
         ...
         */
        if (! $this->attributeFamilies->where('code', "default")->first()) {
            $this->skipRow($rowNumber, self::ERROR_INVALID_ATTRIBUTE_FAMILY_CODE, 'attribute_family_code');

            return false;
        }

        if (! isset($this->typeFamilyValidationRules[$rowData['type']]["default"])) {
            $this->typeFamilyValidationRules[$rowData['type']]["default"] = $this->getValidationRules($rowData);
        }

        /**
         * Validate product attributes
         */
        $validator = Validator::make($rowData, $this->typeFamilyValidationRules[$rowData['type']]["default"]);
        ...
        if ($rowData['type'] == self::PRODUCT_TYPE_CONFIGURABLE) {
            ...
            $familyAttributes = $this->getProductTypeFamilyAttributes($rowData['type'], "default");
    ...
    public function getValidationRules(array $rowData): array
    {
        ...
        $attributes = $this->getProductTypeFamilyAttributes($rowData['type'], "default");
    ...
    public function prepareAttributeValues(array $rowData, array &$attributeValues): void
    {
        ...
        foreach ($rowData as $attributeCode => $value) {
            ...
            if ($attribute->type == 'select') {
                $attributeOption = $this->attributeOptionRepository->where('attribute_id', $attribute->id)
    ...
    public function prepareImages(array $rowData, array &$imagesData): void
    {
        ...
        foreach ($imageNames as $key => $image) {
            $path = 'public/import/'.$this->import->images_directory_path.'/'.$image;
    ...
    {
        $attributeFamily = $this->attributeFamilies->where('code', "default")->first();
    ...
    public function prepareBundleOptions(array $rowData, array &$bundleOptions): void
    {
        ...
        foreach ($options as $option) {
            ...
            if (! isset($bundleOptions[$rowData['sku']]["en"][$attributes['name']])) {
                $productSortOrder = 0;

                $bundleOptions[$rowData['sku']]["en"][$attributes['name']]['attributes'] = [
            ...
            $bundleOptions[$rowData['sku']]["en"][$attributes['name']]['skus'][$attributes['sku']] = [
                'qty'        => $attributes['qty'],
    ...
    protected function prepareRowForDb(array $rowData): array
    {
        ...
        $rowData['locale'] = isset($rowData['locale']) ?? app()->getLocale();

        $rowData['channel'] = isset($rowData['channel']) ?? core()->getDefaultChannelCode();
